...
func repeat(graph [][]int, times int) [][]int {
	w := len(graph[0])
	h := len(graph)

	result := make([][]int, h*times)
	for y := 0; y < h*times; y++ {
		result[y] = make([]int, w*times)
		for x := 0; x < w*times; x++ {
			dx := x / w
			dy := y / h
			d := dx + dy
			result[y][x] = (graph[y%h][x%w]-1+d)%9 + 1
		}
	}
	return result
}
...
func main() {
	...
	// given the repeating structure of the graph, maybe we could figure out a smarter way
	// but for now, let's just use plain dijkstra
	graph := repeat(input, 5)

	println(CheapestPath(graph))
}
